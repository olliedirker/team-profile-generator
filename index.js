...
const htmlCard = (e) => {
  ...
  switch (e.getTeamTitle()) {
    ...
    case Employee.Engineer:
      roleInfo = `<i class="bi-github" role="img" aria-label="GitHub"></i> Github: ${e.getGithubU()}`;
      employeeTypeClasses = "bg-primary bg-gradient text-light";
            ...
            <div class='card-body'>
              <ul class='list-group'>
                <li class='list-group-item'>
                  Role: <strong>${e.getTeamTitle()}</strong>
                </li>
                <li class='list-group-item'>
...
const validation = {
  required: (response) => {
    return response
      ? true
      : console.error("This answer is required to proceed");
  },
  email: (response) => {
    return emailValidator.validate(response)
      ? true
      : console.error(" = you must use a valid email");
  },
};
...
const repeatQuestions = () => {
  inquirer
    ...
    //creates new employees rather than deleted and replacing old one
    .then((answers) => {
